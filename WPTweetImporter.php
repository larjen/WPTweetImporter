...
class WPTweetImporter {
    ...
    static function activation() {
        ...
        update_option("WPTweetImporter_NUMBER_OF_TWEETS", 50);
    ...
    static function add_message($message) {
        ...
        update_option("WPTweetImporter_MESSAGES", $messages);
    }
    ...
    static function start_aggressive_schedule() {
        ...
        wp_schedule_event($start, 'everyMinute', 'WPTweetImporter_update');
    }
    ...
    static function start_moderate_schedule() {
        ...
        wp_schedule_event($start, 'hourly', 'WPTweetImporter_update');
    }

    static function clear_schedule() {
        // unschedule previous schedule
        wp_clear_scheduled_hook('WPTweetImporter_update');
    }
    ...
    static function import_twitter_feed() {
        ...
        if (get_option("WPTweetImporter_ACTIVE") == true) {
            ...
            // fetched all tweets
            $fetchedAllTweets = get_option("WPTweetImporter_FETCHED_ALL_TWEETS");
            ...
            if ($fetchedAllTweets) {
                ...
                $count = get_option("WPTweetImporter_NUMBER_OF_TWEETS");

                // advance page by one this done here since first page is 1 not 0
                $current_page = strval(get_option("WPTweetImporter_CURRENT_PAGE")) + 1;
    ...
    static function update_twitter_feed($current_page, $count) {
        ...
        $connection = new TwitterOAuth(get_option("WPTweetImporter_TWITTER_CONSUMER_KEY"), get_option("WPTweetImporter_TWITTER_CONSUMER_SECRET"), get_option("WPTweetImporter_TWITTER_ACCESS_TOKEN"), get_option("WPTweetImporter_TWITTER_ACCESS_TOKEN_SECRET"));
        $content = $connection->get('account/verify_credentials');
        $timeline = $connection->get("statuses/user_timeline", array("screen_name" => get_option("WPTweetImporter_TWITTER_SCREEN_NAME"), "count" => $count, "page" => $current_page));
        ...
        if (isset($timeline->errors)) {
            ...
            update_option("WPTweetImporter_STATUS", date("Y-m-d H:i:s") . " - Something went wrong, when trying to import tweet " . (($current_page - 1) * $count) . " to " . ((($current_page - 1) * $count) + $count) . ". Error: " . $timeline->errors[0]->message);
            update_option("WPTweetImporter_INFOCLASS", "error");
        } else {
            ...
            if (empty($timeline)) {
                ...
                update_option("WPTweetImporter_CURRENT_PAGE", 0);
                update_option("WPTweetImporter_FETCHED_ALL_TWEETS", true);
                self::add_message("Plugin has imported all of your tweets, and is now actively importing new tweets hourly.");
                update_option("WPTweetImporter_STATUS", date("Y-m-d H:i:s") . " - Plugin has imported all of your tweets, and is now actively importing new tweets hourly.");
                update_option("WPTweetImporter_INFOCLASS", "updated");
                ...
                update_option("WPTweetImporter_CURRENT_PAGE", $current_page);
    ...
    static function insert_twitter_post($tweet) {
        ...
        $post = array(
            ...
            'post_author' => get_option("WPTweetImporter_TWEET_AS"), //The user ID number of the author.
            ...
            'post_category' => array(get_option('WPTweetImporter_TWITTER_CATEGORY')),
    ...
    static function plugin_options() {
        ...
        // save all settings
        $optionsArr = array("WPTweetImporter_TWEET_AS", "WPTweetImporter_TWITTER_CATEGORY", "WPTweetImporter_TWITTER_SCREEN_NAME", "WPTweetImporter_TWITTER_CONSUMER_KEY", "WPTweetImporter_TWITTER_CONSUMER_SECRET", "WPTweetImporter_TWITTER_ACCESS_TOKEN", "WPTweetImporter_TWITTER_ACCESS_TOKEN_SECRET", "WPTweetImporter_NUMBER_OF_TWEETS");
        ...
        if (isset($_POST["WPTweetImporter_PURGE_TWEETS"])) {
            do_action('WPTweetImporter_purge_tweets');
        }

        $force_import_checked = "";
        if (isset($_POST["WPTweetImporter_FORCE_IMPORT"])) {
        ...
        echo '<table class="form-table"><tbody>';
        echo '<tr valign="top"><th scope="row"><label for="TWITTER_SCREEN_NAME">Twitter name (Screen name)</label></th><td><input id="TWITTER_SCREEN_NAME" name="WPTweetImporter_TWITTER_SCREEN_NAME" type="text" value="' . get_option("WPTweetImporter_TWITTER_SCREEN_NAME") . '" class="regular-text"></td></tr>';
        ...
        echo '<tr valign="top"><th scope="row"><label for="TWITTER_CONSUMER_KEY">Consumer key</label></th><td><input id="TWITTER_CONSUMER_KEY" name="WPTweetImporter_TWITTER_CONSUMER_KEY" type="text" value="' . get_option("WPTweetImporter_TWITTER_CONSUMER_KEY") . '" class="regular-text"></td></tr>';
        echo '<tr valign="top"><th scope="row"><label for="TWITTER_CONSUMER_SECRET">Consumer secret</label></th><td><input id="TWITTER_CONSUMER_SECRET" name="WPTweetImporter_TWITTER_CONSUMER_SECRET" type="text" value="' . get_option("WPTweetImporter_TWITTER_CONSUMER_SECRET") . '" class="regular-text"></td></tr>';
        echo '<tr valign="top"><th scope="row"><label for="TWITTER_ACCESS_TOKEN">Access token</label></th><td><input id="TWITTER_ACCESS_TOKEN" name="WPTweetImporter_TWITTER_ACCESS_TOKEN" type="text" value="' . get_option("WPTweetImporter_TWITTER_ACCESS_TOKEN") . '" class="regular-text"></td></tr>';
        echo '<tr valign="top"><th scope="row"><label for="TWITTER_ACCESS_TOKEN_SECRET">Access token secret</label></th><td><input id="TWITTER_ACCESS_TOKEN_SECRET" name="WPTweetImporter_TWITTER_ACCESS_TOKEN_SECRET" type="text" value="' . get_option("WPTweetImporter_TWITTER_ACCESS_TOKEN_SECRET") . '" class="regular-text"></td></tr>';
        echo '</tbody></table>';
...
// register wp hooks
add_action('admin_menu', 'WPTweetImporter::plugin_menu');

// define custom actions
add_action('WPTweetImporter_update', 'WPTweetImporter::import_twitter_feed');
add_action('WPTweetImporter_purge_tweets', 'WPTweetImporter::purge_tweets');
